...
public class Player {
    ...
    private float deathStateTime = 0; // Tiempo desde que comenzÃ³ la animaciÃ³n de muerte
    private boolean isAttacking = false; // Indica si el jugador estÃ¡ atacando
    ...
    private float attackStateTime = 0;  // Tiempo acumulado para la animaciÃ³n de ataque
    ...
    private boolean isBeingPushed = false; // Indica si el jugador está siendo empujado
    private float pushTimer = 0;           // Temporizador para la duración del empuje
    private final float PUSH_DURATION = 0.2f; // Duración del empuje en segundos
    ...
    public void update(float deltaTime, Camera camera, Array<Platform> platforms, Array<Enemy> enemies, Array<Pinchos> pinchos,  Array<Estatua> estatuas, Array<Ground> grounds) {
        ...
        // Actualiza la hitbox segÃºn la posiciÃ³n del personaje
    ...
    private void handleInput(float deltaTime, Array<Enemy> enemies,  Array<Estatua> estatuas) {
        ...
        if (Gdx.input.isKeyJustPressed(Input.Keys.SPACE) && attackCooldown <= 0) {
            isAttacking = true;
            attackStateTime = 0; // Reinicia el tiempo de la animaciÃ³n
     ...
     // Colisiones con enemigos
        ...
        for (Pinchos pincho : pinchos) {
            if (hitbox.overlaps(pincho.getBounds())) {
                if (velocity.y <= 0) { // Si el jugador cae o estÃ¡ sobre los pinchos
        ...
        // Temporizador para el estado de daÃ±o
        if (isHurt) {
    ...
    public void reset() {
        ...
        hurtTimer = 0;  // Reiniciar el temporizador de daÃ±o
        isHurt = false;  // Asegurarse de que no estÃ© en estado de herido
    }
}
